        ...
        <div class="col-sm-6">
            ...
            <select name="port_exportation" id="country_destination" class="form-control select2" required>
                ...
                @foreach($ports as $key => $port)
                    <option value="{{ $key }}" {{ $key == old('country_destination', array_get($trade_permit, 'country_destination')) ? 'selected' : '' }}>{{ $port }}</option>
                @endforeach
            </select>
        </div>
        <div class="col-sm-6">
    ...
    <div class="row">
        <div class="col-sm-6">
            ...
            <select name="port_exportation" id="country_exportation" class="form-control select2" required>
                ...
                @foreach($ports as $key => $port)
                    <option value="{{ $key }}" {{ $key == old('country_exportation', array_get($trade_permit, 'country_exportation')) ? 'selected' : '' }}>{{ $port }}</option>
                @endforeach
            </select>
        </div>
        <div class="col-sm-6">
